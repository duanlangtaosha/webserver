...
//	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA|RCC_APB2Periph_GPIOC|RCC_APB2Periph_AFIO, ENABLE);	 //使能PA,C端口时钟
	...
	GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_4;   			//中断引脚PA1上拉输入
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT; 
...
//	EXTI_InitTypeDef   EXTI_InitStructure;
  ...
  NVIC_Init(&NVIC_InitStructure);//配置
	...
	SPI2_SetSpeed(SPI_BaudRatePrescaler_16);	//SPI1 SCK频率为36M/4=4.5Mhz
	//初始化MAC地址
//	temp=*(vu32*)(0x1FFFF7E8);	//获取STM32的唯一ID的前24位作为MAC地址后三字节
temp=*(vu32*)(0x1FFF7A10);
...
void EXTI2_IRQHandler(void)
{
	...
	OSIntEnter();    
#endif
	EXTI_ClearITPendingBit(EXTI_Line2); //清除中断线6挂起标志位
