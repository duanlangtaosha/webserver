...
u8 ENC28J60_Init(void)
{
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB | RCC_APB2Periph_GPIOC | RCC_APB2Periph_AFIO, ENABLE);	 //使能PA,C端口时钟
    	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_13|GPIO_Pin_14|GPIO_Pin_15;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP ;   	//推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOB, &GPIO_InitStructure);
 	GPIO_SetBits(GPIOB,GPIO_Pin_13|GPIO_Pin_14|GPIO_Pin_15);	//PA2,3,4置高
 	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10 | GPIO_Pin_11;				//PC4 推挽 	  
 	GPIO_Init(GPIOC, &GPIO_InitStructure);
	GPIO_SetBits(GPIOC,GPIO_Pin_10 | GPIO_Pin_11);							//PC4上拉
	
	GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_12;   			//中断引脚PC12上拉输入
	...
	EXTI_ClearITPendingBit(EXTI_Line12); //清除中断线1挂起标志位
	
	NVIC_InitStructure.NVIC_IRQChannel = EXTI15_10_IRQn;			//外部中断线1
